...
data_df = pd.read_csv(os.getcwd()+"\datasets\DataSources/by_location.csv")
...
mapdiagram = folium.Map(location=[20, 0], tiles="OpenStreetMap", zoom_start=2)
...
continent_value_counts = (
    data_df["Continent"].value_counts()/50).astype("int32")
...
top10_artists = pd.Series(Artists).value_counts()[
...
top5_duration
filtered_data1 = data_df[(data_df["Country"].isin(list(
    top5_duration.index))) + (data_df["Country"].isin(list(bottom5_duration.index)))]
...
sns.boxplot(x=filtered_data1["Country"],
            y=filtered_data1["duration_in_s"], color='#1db954')
